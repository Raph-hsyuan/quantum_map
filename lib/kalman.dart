...
class KalmanFilter {
  ...
  void addObservation(num observationWeight, num y, List<double> observation) {
  ...
  void _updateBeta(num y, List<double> observation) {
    num predicted = 0.0;
    for (int i = 0; i < dimension; i++) predicted += beta[i] * observation[i];
    ...
    //update beta
    for (int i = 0; i < dimension; i++) beta[i] = beta[i] + weightedResidual[i];
  }
  ...
  void _updateKGains(List<double> observation, num weight) {
    ...
    num denominator = 0.0;
    for (int i = 0; i < dimension; i++) denominator += observation[i] * px[i];
  ...
  _forget(num trace) {
    if (trace < maxTraceToStopForgetting &&
  ...
  void _updateCovarianceP(List<double> observation) {
    ...
    for (int i = 0; i < dimension; i++) toMultiply[i] = new List(dimension);
    ...
    for (int i = 0; i < dimension; i++)
      for (int j = 0; j < dimension; j++) {
        ...
        if (i == j)
          toMultiply[i][j] +=
              1; //diagonal element needs to be summed to a diag(1)
      }
